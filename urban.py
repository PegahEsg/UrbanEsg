...
def optimize(input):
    ...
    longest_length=round((Lengths * u * 2) + (Widths * v) + (Sub_street * ((u *2) - 1)), 2) #round(max((u*Widths)+Lengths,(v*Lengths)+Widths),2)
...
if on:
    ...
    if not algorithm.result:
        st.error("No results were generated by the algorithm. Please check your optimize() function.")
    else:
        try:
            ...
            st.error(f"Error creating scores dataframe: {e}")    
